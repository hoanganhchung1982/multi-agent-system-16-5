...
// 2. KHỞI TẠO (Sửa lỗi Vite biến môi trường)
...
// 5. AUDIO (Sử dụng Web Speech API cho ổn định khi build)
export const fetchTTSAudio = async (text: string) => {
  return text; // Trả về text để hàm play xử lý trực tiếp
};

export const playStoredAudio = (text: string) => {
  return new Promise((resolve) => {
    if (!('speechSynthesis' in window)) return resolve(null);
    window.speechSynthesis.cancel(); // Hủy các âm thanh đang phát
